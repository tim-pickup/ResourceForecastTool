...
import type { DemandItem, SkillRequirement, NamedAllocation, Level, DemandStatus } from '../../types'
import { Button } from '../../components/ui/Button'
import { formatMonthLabel } from '../../utils/capacity'
...
function CoverageStrip({ coverage }: { coverage: MonthCoverage[] }) {
  return (
    <div className="flex gap-0.5 flex-wrap">
      {coverage.map(({ month, target, allocated, status }) => (
        <div
          ...
          className={clsx(
            'h-2 rounded-sm flex-1 min-w-[20px]',
...
function AllocationRow({ alloc, req, months, onChange, onDelete }: AllocationRowProps) {
  const { people, skills } = useAppStore()
  const [showAll, setShowAll] = useState(false)

  const skill = skills.find(s => s.id === req.skill_id)
  const eligibleIds = new Set(
    people.filter(p => p.active && p.skills.some(ps => ps.skill_id === req.skill_id && meetsLevel(ps.level, req.level))).map(p => p.id)
  )

  const visiblePeople = showAll ? people.filter(p => p.active) : people.filter(p => p.active && (eligibleIds.has(p.id) || p.id === alloc.person_id))
  const selectedPerson = people.find(p => p.id === alloc.person_id)
...
function RequirementAllocationBlock({ req, months, onChange }: ReqBlockProps) {
  const { skills, themes } = useAppStore()

  const skill = skills.find(s => s.id === req.skill_id)
  const theme = skill ? themes.find(t => t.id === skill.theme_id) : null
  const coverage = requirementCoverage(req, months)
  ...
  const addAlloc = () => {
    ...
    onChange({ ...req, allocations: [...req.allocations, { id: generateId('alloc'), person_id: '', hours_by_month: hbm, notes: null }] })
  }

  const targetTotal = months.reduce((s, m) => s + (req.hours_by_month[m] ?? 0), 0)
  const allocTotal = req.allocations.reduce((s, a) => s + months.reduce((ss, m) => ss + (a.hours_by_month[m] ?? 0), 0), 0)
    ...
    <div className="border border-border rounded overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-gray-50">
        ...
        <div className="text-xs text-gray-500">
          ...
          <span>{Math.round(targetTotal)}h</span>
        </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-gray-50 border border-border rounded p-3 text-xs flex flex-col gap-1">
        ...
        <div className="text-gray-400 text-[10px]">
          {draft.phases.length} phase{draft.phases.length !== 1 ? 's' : ''} · {draft.phases.map(p => `${p.name} (${p.start_month}–${p.end_month})`).join(', ')}
        </div>
      </div>
      ...
      {/* Phase allocation sections */}
      {draft.phases.map(phase => {
        const months = getMonths(phase.start_month, phase.end_month)
        return (
          <div key={phase.id}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-semibold text-gray-700">{phase.name || 'Unnamed Phase'}</span>
