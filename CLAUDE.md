## Project Summary
Resourcing planning tool to forecast skillsets by Person/Project/Skill.
...
- ALWAYS use `claude-flow` directly, never `npx @claude-flow/cli@latest`
- Correct: `claude-flow swarm init --topology hierarchical`
- Incorrect: `npx @claude-flow/cli@latest swarm init`
- Reason: global install required on Windows due to npm Invalid Version bug
...
- Run `claude-flow security scan` after security-related changes
...
claude-flow memory store --key "pattern-auth" --value "JWT with refresh" --namespace patterns
